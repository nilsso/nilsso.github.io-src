...
\begin{figure}[h]
  \begin{tikzpicture}[every node/.style={draw,shape=circle,fill=white,inner sep=1}]
    \path
      (.5,.5) node (a) {}
...
\begin{figure}[h]
  ...
  \begin{subfigure}[t]{0.3\textwidth}
    \centering
    \begin{tikzpicture}[
        every node/.style={draw,shape=circle,fill=white,inner sep=1},
...
\begin{figure}[h]
  \begin{tikzpicture}[
    dot/.style={draw,shape=circle,fill=white,inner sep=1},
    decoration={markings,mark=at position 0.5 with {\arrow{>}}}
  ]
